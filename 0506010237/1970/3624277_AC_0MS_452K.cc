...
int findx(int i,int j,int k){
    ...
    if(num[i][j].v!=k) return 0;
    num[i][j].x=true;
    return 1+findx(i,j+1,k); 
}
int findy(int i,int j,int k){
    if(i>18) return 0;
    if(num[i][j].v!=k) return 0;
    num[i][j].y=true;
    return 1+findy(i+1,j,k); 
}
void find(){
    for(int i=0;i<19;i++)
       for(int j=0;j<19;j++){
             ...
             if(j<15){
                  ...
                  if(num[i][j].v==2 && !num[i][j].x && findx(i,j,2)==5){ cout<<2<<endl<<i+1<<" "<<j+1<<endl; return;} 
             }
             if(i<15){
                  if(num[i][j].v==1 && !num[i][j].y && findy(i,j,1)==5){ cout<<1<<endl<<i+1<<" "<<j+1<<endl; return;} 
                  if(num[i][j].v==2 && !num[i][j].y && findy(i,j,2)==5){ cout<<2<<endl<<i+1<<" "<<j+1<<endl; return;} 
             }      
       }  
       cout<<0<<endl;   
}
