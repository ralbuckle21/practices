...
using namespace std;
int sum=0;
int net(int s,int t,int cost[701][701],int n){//是通过最短路径所改
     bool visit[701];
     int u,dist[701],path[701];//dist记录的是从起点到其它点的最少距离path记录点的前驱
     for(int i=1;i<=n;i++){
        visit[i]=0;
        dist[i]=cost[s][i]; 
        path[i]=s;      
...
void total(){
    int n,t,totals=0,max=0,cost[701][701],nums[701][701],sign[701];
    scanf("%d",&t);
    while(t--){//可接受多组测试语句
         scanf("%d",&n);
