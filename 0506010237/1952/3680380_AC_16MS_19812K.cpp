...
int list(){
     ...
     for(int i=1;i<n;i++){
         ...
         if(p>len){
         ...
         else if(num[i]>b[p].v[b[p].slen-1][0]){
               int j=0;
               ...
               int j=0;
               b[p].v[b[p].slen-1][1]=b[p-1].v[b[p-1].slen-1][1];
               if(b[p].slen>=2) b[p].v[b[p].slen-1][1] += b[p].v[b[p].slen-2][1];
               while(num[i]>=b[p-1].v[j][0]) j++;
               j--;
               if(j>=0) b[p].v[b[p].slen-1][1] -= b[p-1].v[j][1]; 
         }
     }
     return len;
}
