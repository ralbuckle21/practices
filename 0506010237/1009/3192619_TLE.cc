...
inline void findup(int *min,int sum,int i,int j)
{
       int  k=i,m=j;
       while(k>0 && m<=n)
       {
...
inline void finddown(int *min,int sum,int i,int j)
{
       int  k=i,m=j;
       while(k<ce && m<n)
       {
        ...
        for(int i=0;i<front;i++)
        {
           ...
           for(int j=1;j<=re[i].count;j++)
           {
               ...
               if(ce==1 && t==0 && fh==1)
               {
                    ...
                    sum=j+f;
                    t=1;
                    result[rear].count=result[rear].count+fs-fh;   
                    continue;       
               }
               if(fh>n && t==0)
               {
