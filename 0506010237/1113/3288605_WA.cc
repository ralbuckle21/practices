...
void Graham_scan(pointt point[1000],int n)
{
    ...
    double x=0;
    for(i=1;i<len;i++)
       x=x+sqrt(((ch[i].x-ch[i-1].x)*(ch[i].x-ch[i-1].x))+((ch[i].y-ch[i-1].y)*(ch[i].y-ch[i-1].y)));
     x=x+(int)sqrt(((ch[len-1].x-ch[0].x)*(ch[len-1].x-ch[0].x))+((ch[0].y-ch[len-1].y)*(ch[0].y-ch[len-1].y)));
     sum=2*acos(-1.0)*l+x;
     printf("%.0lf\n",sum);
}
int main()
